...
data <- load_and_clean_data(763628)
library(MCMCvis)
...
saveRDS(model, "analyses/model-fit_results/763628/commonk.rds")
#model_fit_results <- model_fit_results[-nrow(model_fit_results), ]
model <- readRDS("analyses/model-fit_results/763628/commonk.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "Normative",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/normative.rds")
model <- readRDS("analyses/model-fit_results/763628/normative.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "VICS",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/vics.rds")
model <- readRDS("analyses/model-fit_results/763628/vics.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "VII",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/vii.rds")
model <- readRDS("analyses/model-fit_results/763628/vii.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "GoMRT",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/gomrt.rds")
model <- readRDS("analyses/model-fit_results/763628/gomrt.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "DLCSL",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/dlcsl.rds")
model <- readRDS("analyses/model-fit_results/763628/dlcsl.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "DLCTL",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/dlctl.rds")
model <- readRDS("analyses/model-fit_results/763628/dlctl.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "MHM",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/mhm.rds")
model <- readRDS("analyses/model-fit_results/763628/mhm.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "ILCRE",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/ilcre.rds")
model <- readRDS("analyses/model-fit_results/763628/ilcre.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "ILCRI",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/763628/ilcri.rds")
model <- readRDS("analyses/model-fit_results/763628/ilcri.rds")



saveRDS(model_fit_results,"analyses/model-fit_results/763628/model_fit_results.rds")

#===========================================================================
#                            Model Fit results table
#===========================================================================
model_fit_results <- readRDS("analyses/model-fit_results/763628/model_fit_results.rds")
library(tinytable)
...
caption = "Model fit results from MCMC analysis for all models applied to Assessment763628 data") |>
...
saveRDS(rank_order, "763628rankorder.rds")
#===========================================================================
#                            MCMC Coverage Table 
#===========================================================================
...
saveRDS(coverage_results,"analyses/model-fit_results/763628/coverage_results.rds")
library(tinytable)
coverage_results<- coverage_results |> 
  mutate_at(vars("C", "eta","kappa","xi"), as.numeric)
tt(coverage_results,
    caption = "MCMC parameter coverage based on Rhat statistics for Assessment 763628") |>
...
library(ggpubr)
full<- plot_engagement_disagreement(C_raw_disagreement, color="#155F83FF", element = element_blank()) + theme(aspect.ratio = 4.4)
C_threshold_disagreement <- C_commonk + C_normative + C_vics + C_vii
threshold_plots <- plot_engagement_disagreement(C_threshold_disagreement,color_fill = "#155F83FF") + theme(aspect.ratio =4.6)

C_latent_diagreement <- C_gomrt+ C_dlcsl + C_dlctl + C_mhm + C_ilcre + C_ilcri
latent_plots<- plot_engagement_disagreement(C_latent_diagreement,color_fill = "#155F83FF",
                        element = element_blank()) + 
    scale_x_discrete(labels = colnames(C_raw_disagreement)) + theme(aspect.ratio = 4.4)
library(knitr)
library(ggpubr)

ggsave("763628disagreementSORTED.pdf")
plot_crop("763628disagreementSORTED.pdf")

ggarrange(threshold_plots,latent_plots, full, ncol = 3, labels = c("A","B","C"),
 font.label = list(size = 12, face = "bold"), hjust = -1.25, vjust = 1.8)
ggplot()
...
melted_mat <- melt(kappa_matrix)
  ...
  scale_fill_gradient(low="white", high="#C16622FF") + geom_text(aes(label = sprintf("%.2f",value)),  # Format to 2 decimal places
            ...
            labs(x="", y = "")
ggsave("763628kappa_agreement.pdf")
plot_crop("763628kappa_agreement.pdf")
kappa_matrix
ratings_data
...
# Optional: Save the plot
 ggsave("Updated_kappa_heatmap763628.pdf", kappa_heatmap, width = 15, height = 8)
 plot_crop("Updated_kappa_heatmap763628.pdf")
...
ggarrange(p_commonk, p_normative, p_vics, p_vii,
          ...
          common.legend = TRUE, ncol=2, nrow=5, label.x = 1,heights = c(1, 1, 1, 1, 1.15))
ggsave("763628proportion_correct.pdf")
plot_crop("763628proportion_correct.pdf")
...
saveRDS(ability_estimates_compare_plot, "763628ability.rds")
...
saveRDS(d_estimates_compare_plot, "763628difficulty.rds")
ggarrange(ability_estimates_compare_plot, d_estimates_compare_plot, nrow = 2, 
            labels = c("A","B"), widths = c(1,1), heights = c(1,1), vjust = 2)
ggsave("763628estimates_compare.pdf")
library(knitr)
plot_crop("763628estimates_compare.pdf")
