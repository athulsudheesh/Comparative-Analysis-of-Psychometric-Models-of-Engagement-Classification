...
data <- load_and_clean_data(735540)
library(MCMCvis)
...
saveRDS(model, "analyses/model-fit_results/735540/commonk.rds")
#model_fit_results <- model_fit_results[-nrow(model_fit_results), ]
model <- readRDS("analyses/model-fit_results/735540/commonk.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "Normative",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/normative.rds")
model <- readRDS("analyses/model-fit_results/735540/normative.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "VICS",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/vics.rds")
model <- readRDS("analyses/model-fit_results/735540/vics.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "VII",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/vii.rds")
model <- readRDS("analyses/model-fit_results/735540/vii.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "GoMRT",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/gomrt.rds")
model <- readRDS("analyses/model-fit_results/735540/gomrt.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "DLCSL",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/dlcsl.rds")
model <- readRDS("analyses/model-fit_results/735540/dlcsl.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "DLCTL",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/dlctl.rds")
model <- readRDS("analyses/model-fit_results/735540/dlctl.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "MHM",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/mhm.rds")
model <- readRDS("analyses/model-fit_results/735540/mhm.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "ILCRE",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/ilcre.rds")
model <- readRDS("analyses/model-fit_results/735540/ilcre.rds")
...
model_fit_results <- rbind(model_fit_results, data.frame("Model" = "ILCRI",
                        ...
                        "WAIC"=model$posterior_samples$WAIC$WAIC,
                        "MSE"= model$cvvals$CVvalue,"SE"= model$cvvals$CVstandardError))
saveRDS(model, "analyses/model-fit_results/735540/ilcri.rds")
model <- readRDS("analyses/model-fit_results/735540/ilcri.rds")



saveRDS(model_fit_results,"analyses/model-fit_results/735540/model_fit_results.rds")

#===========================================================================
#                            Model Fit results table
#===========================================================================
model_fit_results <- readRDS("analyses/model-fit_results/735540/model_fit_results.rds")
library(tinytable)
...
saveRDS(rank_order, "735540rankorder.rds")
#===========================================================================
#                            MCMC Coverage Table 
#===========================================================================
...
saveRDS(coverage_results,"analyses/model-fit_results/735540/coverage_results.rds")
library(tinytable)
coverage_results<- coverage_results |> 
  mutate_at(vars("C", "eta","kappa","xi"), as.numeric)
tt(coverage_results,
    caption = "MCMC parameter coverage based on Rhat statistics for Assessment 735540") |>
     format_tt(j = 1:7, digits = 2,num_fmt = "decimal", replace = TRUE) |>
     style_tt(i =5:7, j = 4, color="red") |>
     style_tt(i =9, j = c(2,4,5,6,7), color="red") |>
     style_tt(i =10, j = c(4,5,6,7), color="red") |>
     print("latex")
...
ggsave("dlctl_engagement_matrix.pdf")
...
library(ggpubr)
ggsave("735540disagreementSORTED.pdf")
full<- plot_engagement_disagreement(C_raw_disagreement, color="#155F83FF", element = element_blank()) + theme(aspect.ratio = 3.5, axis.text.y = element_text(size=8))
C_threshold_disagreement <- C_commonk + C_normative + C_vics + C_vii
threshold_plots <- plot_engagement_disagreement(C_threshold_disagreement,color_fill = "#155F83FF") + theme(aspect.ratio =3.7, axis.text.y = element_text(size=8))

C_latent_diagreement <- C_gomrt+ C_dlcsl + C_dlctl + C_mhm + C_ilcre + C_ilcri
latent_plots<- plot_engagement_disagreement(C_latent_diagreement,color_fill = "#155F83FF",
                        element = element_blank()) + 
    scale_x_discrete(labels = colnames(C_raw_disagreement)) + theme(aspect.ratio =3.5, axis.text.y = element_text(size=8))
library(knitr)
library(ggpubr)

ggsave("735540disagreement.pdf")
plot_crop("735540disagreementSORTED.pdf")

ggarrange(threshold_plots,latent_plots, full, ncol = 3, labels = c("A","B","C"),
 font.label = list(size = 12, face = "bold"), hjust = -0.01)
ggplot()
...
melted_mat <- melt(kappa_matrix)
  ...
  scale_fill_gradient(low="white", high="#C16622FF") + geom_text(aes(label = sprintf("%.2f",value)),  # Format to 2 decimal places
            ...
            labs(x="", y = "")
ggsave("735540kappa_agreement.pdf")
plot_crop("735540kappa_agreement.pdf")
kappa_matrix
ratings_data
...
# Optional: Save the plot
 ggsave("Updated_kappa_heatmap735540.pdf", kappa_heatmap, width = 15, height = 8)
 plot_crop("Updated_kappa_heatmap735540.pdf")
...
p_commonk<- plot_proportion_correct(X, C_commonk)+ labs(title="Common K") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                       axis.title.x = element_blank(size=12),
                                                                                       axis.title.y = element_blank(size=6)) 
p_normative<- plot_proportion_correct(X, C_normative)+ labs(title="Normative") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=6))
p_vics<- plot_proportion_correct(X, C_vics)+ labs(title="VICS") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=6)) 
p_vii<- plot_proportion_correct(X, C_vii)+ labs(title="VII") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=6)) 
p_gomrt<- plot_proportion_correct(X, C_gomrt)+ labs(title="GoMRT") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=6)) 
p_dlcsl<- plot_proportion_correct(X, C_dlcsl)+ labs(title="DLCSL") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=6)) 
p_dlctl<- plot_proportion_correct(X, C_dlctl)+ labs(title="DLCTL") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=6)) 
p_mhm<- plot_proportion_correct(X, C_mhm)+ labs(title="MHM") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=3))
p_ilcre<- plot_proportion_correct(X, C_ilcre)+ labs(title="ILCRE") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=3)) 
p_ilcri<- plot_proportion_correct(X, C_ilcri) + labs(title="ILCRI") + theme(plot.title = element_text(hjust=0.5, size = 14),
                                                                                      axis.title.x = element_blank(size=12),
                                                                                      axis.title.y = element_blank(size=3)) 
# For left column plots (p_commonk, p_vics, p_gomrt, p_mhm, p_ilcre)
# Keep the y-axis text and add margins
left_theme <- theme(
...
# For right column plots (p_normative, p_vii, p_dlcsl, p_dlctl, p_ilcri)
# Remove y-axis text and add margins
right_theme <- theme(
  plot.title = element_text(hjust=0.5, size = 14),
  axis.title.x = element_blank(),
  axis.title.y = element_blank(),
...
# Apply these themes to your plots
p_commonk <- plot_proportion_correct(X, C_commonk) + labs(title="Common K") + left_theme
p_normative <- plot_proportion_correct(X, C_normative) + labs(title="Normative") + right_theme
p_vics <- plot_proportion_correct(X, C_vics) + labs(title="VICS") + left_theme
# Apply to other plots accordingly
# Adjust the size of the output to ensure proper spacing
ggarrange(p_commonk, p_normative, p_vics, p_vii,
          p_gomrt, p_dlcsl, p_dlctl,
          p_mhm, p_ilcre, p_ilcri, 
          common.legend = TRUE, ncol=2, nrow=5,
          label.x = 1, heights = c(1.5, 1.5, 1.5, 1.5, 1.7))

ggsave("735540proportion_correct.pdf", width = 10, height = 14)

ggarrange(p_commonk, p_normative, p_vics, p_vii,
          p_gomrt, p_dlcsl, p_dlctl,
          p_mhm, p_ilcre, p_ilcri, 
          common.legend = TRUE, ncol=2, nrow=5, label.x = 1,heights = c(1.5, 1.5, 1.5, 1.5, 1.7))
ggsave("735540proportion_correct.pdf")
plot_crop("735540proportion_correct.pdf")
...
saveRDS(ability_estimates_compare_plot, "735540ability.rds")
...
saveRDS(d_estimates_compare_plot, "735540difficulty.rds")

ggarrange(ability_estimates_compare_plot, d_estimates_compare_plot, nrow = 2, 
            labels = c("A","B"), widths = c(1,1), heights = c(1,1), vjust = 1)
ggsave("735540estimates_compare.pdf")
library(knitr)
plot_crop("735540estimates_compare.pdf")
